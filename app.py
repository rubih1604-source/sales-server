...
def get_credentials():
    ...
    return Credentials(token=token_data.get('token'),refresh_token=token_data.get('refresh_token'),token_uri="https://oauth2.googleapis.com/token",client_id=CLIENT_ID,client_secret=CLIENT_SECRET,scopes=SCOPES)

def save_credentials(creds):
    with open(TOKEN_FILE,'w') as f: json.dump({'token':creds.token,'refresh_token':creds.refresh_token},f)
...
def index():
    creds = get_credentials()
    st = "✅ מחובר" if creds else "❌ לא מחובר"
    link = "" if creds else "<br><a href='/oauth/start' style='background:#3b82f6;color:white;padding:12px 24px;border-radius:8px;text-decoration:none'>התחבר ל-Gmail</a>"
...
def oauth_callback():
    ...
    return "<html dir='rtl'><body style='font-family:Arial;padding:40px;background:#0d1117;color:#e6edf3'><h1>✅ התחברות הצליחה!</h1><p style='color:#10b981'>המערכת מחוברת.</p></body></html>"
...
def parse_sale_from_thread(messages, thread_id):
    ...
    sale_keywords = ['תואם','לאשר חוזה','ממירים','דאבל יס','דרבל יס','שובץ','להקים','ממיר']
    ...
    # ביטול
    is_cancelled = any(p in full_text for p in ['לא להקים','התחרטה','לבטל','ביטול','מבטל'])
    cancel_note = 'התחרטה' if 'התחרטה' in full_text else ('לא להקים' if 'לא להקים' in full_text else '')
    ...
    for pat in patterns:
        for match in re.finditer(pat,full_text):
            all_found.append((match.start(),match.group(1),match.group(2)))
    ...
    if all_found:
        ...
        if len(all_found)>1:
            ...
            if first[2] != latest[2]:
                has_change = True
                change_note = f'שונה מ-{first[2]} ל-{latest[2]}'
    ...
    return {'name':subj,'customerId':cid,'installDate':install_date,'installMonth':install_month,'hours':install_hours,'mirrors':mirrors,'status':status,'hasChange':has_change,'changeNote':change_note,'isCancelled':is_cancelled,'cancelNote':cancel_note,'isToday':False,'isApril':is_april,'threadId':thread_id}

@app.route('/api/scan')
def scan():
    service = get_service()
    if not service:
        return jsonify({'error':'not_authenticated','auth_url':'https://sales-rubi.onrender.com/oauth/start'}),401
    try:
        sales,invoices = [],[]
        ...
        inv_results = service.users().messages().list(userId='me',
            q='(חשבונית OR invoice OR receipt OR morning.co OR render.com OR cardcom) has:attachment after:2026/1/1',
        ...
        for msg in inv_results.get('messages',[]):
            try:
                ...
                biz_keywords = ['ר.א.מ','ליד מנג','render','funnelly','stripe','morning','חשבונית ירוקה','atp','icount','חשבונית מס','קבלה']
                if not any(k.lower() in (subj+sender).lower() for k in biz_keywords): continue
                atts = []
                find_attachments(md['payload'],atts,msg['id'])
                inv_type = 'income' if any(k in (subj+sender) for k in ['ר.א.מ','עמלות','may zalah']) else ('subscription' if any(k in (subj+sender) for k in ['חשבונית ירוקה','morning','חשבון חודשי']) else 'expense')
                num_m = re.search(r'(\d{4,})',subj)
                invoices.append({'id':msg['id'],'date':date,'month':month,'from':re.match(r'^"?([^"<]+)',sender).group(1).strip() if sender else '','subject':subj,'description':subj[:50],'invoiceNum':num_m.group(1) if num_m else '','type':inv_type,'amount':None,'currency':'ILS','gmailLink':f'https://mail.google.com/mail/u/0/#all/{msg["id"]}','hasAttachment':len(atts)>0,'attachments':atts})
            except: continue

        sales.sort(key=lambda x:(x.get('installMonth',''),x.get('installDate','')),reverse=True)
        return jsonify({'success':True,'sales':sales,'invoices':invoices,'counts':{'sales':len(sales),'invoices':len(invoices)}})
...
def send_to_accountant():
    ...
    try:
        ...
        for inv in invoices:
            body += f'• {inv.get("from","")} — {inv.get("description","")}\n'
        body += '\nבברכה,\nרובי חגג'
